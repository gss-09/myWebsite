...
- `src/components/cli/` games — `Snake Tetris Pong Breakout Game2048 TypingTest`, each a canvas/grid component taking an `onExit` prop. `Cli.js` also has an inline number-guess game. **Each game has its own retro-hardware identity** (Snake = Nokia 3310 LCD, Tetris = Game Boy DMG, Pong = 1972 b/w, Breakout = Atari 2600 rainbow, 2048 = heat ramp, typing = teletype paper) — palettes are hardcoded per game and deliberately independent of the terminal theme; don't recolor them with `--amber` or add phosphor glow. Shared kit in `cli/arcade.js`: `sfx` (WebAudio synth, mute in localStorage `arcade-mute`), `useBest` (high scores, `arcade-best-*`), `TitleCard`, particles/shake/block-digit helpers. Per-cabinet CSS lives under `.cab-*` classes in `index.css`.
